...
**A**：可以只部署一个产品，在一键部署界面可以选择

**Q3**：云盘创建的大小在哪里配置的？

**A**：在“组件&应用一键部署模板\(全中心应用\).xlsx”文件中的云盘定义的页签中新增了云盘大小的设置栏

**Q4**：技术组件的安装是在哪里定义的？

**A**：在“组件&应用一键部署模板\(全中心应用\).xlsx”文件中**midware deploy页签**定义的
...
payc-web-product 云盘里面有关联的文件，需要根据其他应用的镜像的内容生成，原来的方案会导致一键部署无法完成，此单作为跟踪此问题，请协调处理

* [ ] 1350452

payc业务需要在一键部署执行完预安装脚本后，创建分区表，需要讨论此分区表的创建方式和环节，如是否是payc侧提供cli程序有devops这边集成，还是payc业务自行去处理此问题
